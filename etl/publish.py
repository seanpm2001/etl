...
@click.option("--catalog", type=click.Path(exists=True), help="Path to catalog", default=DATA_DIR)
def publish(dry_run: bool, bucket: str, catalog: Path) -> None:
    """
    Publish the generated data catalog to S3.
    """
    sanity_checks(Path(catalog))
    sync_catalog_to_s3(bucket, Path(catalog), dry_run=dry_run)
...
def sync_datasets(s3: Any, bucket: str, catalog: Path, dry_run: bool = False) -> None:
    ...
    for ds in local.iter_datasets():
        ...
        print("-", path)
        if not dry_run:
            sync_folder(s3, bucket, catalog, catalog / path, path)
...
def sync_folder(
    s3: Any, bucket: str, catalog: Path, local_folder: Path, dest_path: str, delete: bool = True
) -> None:
    """
...
def get_published_checksums(bucket: str) -> Dict[str, str]:
    ...
    try:
        existing = pd.read_feather(
            f"https://{bucket}.{config.S3_HOST}/catalog.feather"
        )
