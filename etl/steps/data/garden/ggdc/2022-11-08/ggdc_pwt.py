...
def run(dest_dir: str) -> None:
    log.info("ggdc_pwt.start")

    # read dataset from meadow
    ds_meadow = Dataset(
        DATA_DIR
        / "meadow/ggdc/2022-11-08/ggdc_pwt"
    )
    tb_meadow = ds_meadow["ggdc_pwt"]

    df = pd.DataFrame(tb_meadow)

    log.info("ggdc_pwt.exclude_countries")
    df = exclude_countries(df)

    log.info("ggdc_pwt.harmonize_countries")
    ...
    tb_garden.update_metadata_from_yaml(N.metadata_path, "ggdc_pwt")
    
    ds_garden.add(tb_garden)
    ds_garden.save()

    log.info("ggdc_pwt.end")
