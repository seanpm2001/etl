...
def run(dest_dir: str) -> None:
    ...
    ds_meadow = Dataset(
        DATA_DIR
        / "meadow/ggdc/2022/pwt"
    )
    tb_meadow = ds_meadow["pwt"]

    df = pd.DataFrame(tb_meadow)

    log.info("pwt.exclude_countries")
    df = exclude_countries(df)

    log.info("pwt.harmonize_countries")
    ...
    tb_garden.update_metadata_from_yaml(N.metadata_path, "pwt")
